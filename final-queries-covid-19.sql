...
CREATE VIEW "Most Infected Countries Percentage" AS
	SELECT Location, Population, MAX(CAST(COALESCE(total_cases,0) AS BIGINT)) AS highest_infections, (MAX(CAST(COALESCE(total_cases,0) AS BIGINT))/population)*100 AS population_infected_percent
	FROM "Covid-19 Dashboard"..CovidDeaths															
	WHERE total_cases <> 'total_cases' AND continent IS NOT NULL
...
CREATE VIEW "Most Infected Countries Percentage and Date" AS
	SELECT Location, Population, date,  MAX(CAST(COALESCE(total_cases,0) AS BIGINT)) AS highest_infections, (MAX(CAST(COALESCE(total_cases,0) AS BIGINT))/population)*100 AS population_infected_percent
	FROM "Covid-19 Dashboard"..CovidDeaths
	WHERE total_cases <> 'total_cases' AND continent IS NOT NULL
